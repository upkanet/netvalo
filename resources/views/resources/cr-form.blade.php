...
@section('middle')
<h3>{{$company->name}} <small>@if(isset($cr)) Modifier @else Ajouter @endif le Compte de R&eacute;sultats {{$year}}</small></h3>
@if(isset($cr))
...
<input type="hidden" name="year" value="{{$year}}">
	<div class="row">
		...
		<div class="col-lg-3 col-sm-6">
			<a class="btn btn-default" href="{{route('companies.show',$company)}}">Retour</a>
			@if(isset($cr))
			<a class="btn btn-danger" id="deleteBilanCRbtn" data-token="{{ csrf_token()}}">Supprimer</a>
			@endif
		</div>
	</div>
	<br>
